function gameload() {
  ...
  var b87 = document.getElementById("bbbb87");
  //取得指定的canvas物件
  ...
  function rightMouseUp(event) {
    if (event.button == 2) goStore = false;
  }
    ...
    switch (event.keyCode) {
      case 52:
        if (ObjectWord.state == false) {
          ...
          setTimeout(
            "ObjectWord.state=false;ObjectWord.x=-99;ObjectWord.y=-99;",
            1800
          );
        }
        break;
  ...
  function Character() {
    ...
    this.draw = function () {
      ...
      LSctx.fill();
      LSctx.drawImage(b87, this.x - 100, this.y - 100, 200, 200);
      LSctx.closePath();
  ...
  enemy = new (function () {
    ...
    this.draw = function () {
      if (
        ...
        this.Ruse = true;
      else this.Ruse = false;

      if (this.Ruse) Sctx.strokeStyle = "#ff0000";
      else Sctx.strokeStyle = "#000000";
      ...
      Sctx.fill();
      Sctx.drawImage(img, this.x - 25, this.y - 25, 50, 50);
      Sctx.closePath();
      ...
      if (this.fly == false) {
        ...
        this.Sx = Math.sqrt(
          Math.pow(this.distance, 2) /
            (Math.pow(this.y - player.y, 2) + Math.pow(this.x - player.x, 2))
        );
        this.distanceX = Math.sqrt(
          Math.pow(this.distance, 2) -
            Math.pow((this.y - player.y) * this.Sx, 2)
        );
        this.distanceY = Math.sqrt(
          Math.pow(this.distance, 2) -
            Math.pow((this.x - player.x) * this.Sx, 2)
        );
        ...
        if (this.x - player.x > 0) {
          ...
          if (
            Math.abs(this.targetPositionX - this.x) <
              this.distanceX / this.Time ||
            this.targetPositionX - this.x <= 0
          )
        ...
        if (this.y - player.y > 0) {
          ...
          if (
            Math.abs(this.targetPositionY - this.y) <
              this.distanceY / this.Time ||
            this.targetPositionY - this.y <= 0
          )
  ...
  ObjectWord = new (function () //////////////////////////////////(李星位置X-滑鼠位置X)^2 + (李星位置Y-滑鼠位置Y)^2 的開根號(斜邊) < 我要設的範圍(長度) 才可以放招式
  {
    ...
    this.draw = function (mouseX, mouseY) {
      ...
      if (this.state == true) {
        ...
        if (this.use) Sctx.strokeStyle = "#ff0000";
        else Sctx.strokeStyle = "#000000";
